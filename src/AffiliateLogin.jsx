      ...
      <div className="login-card" style={{maxWidth:'420px',width:'100%',borderTop:'4px solid #1b6a2f',margin:0}} onClick={e=>e.stopPropagation()}>
        ...
        {!forgotSent ? <>
          ...
          <div style={{marginTop:'14px',padding:'12px',borderRadius:'10px',background:'#f1faf2',border:'1px solid #cde8ce',fontSize:'12.5px',color:'#0f2a1a',lineHeight:1.6}}>
            ...
            If an account exists for <strong>{forgotEmail}</strong>, a verification link has been sent to your <strong>registered mail</strong> via Gmail SMTP. Open the email and follow the reset instructions. The link expires in 15 minutes.
            {forgotLink && <div style={{marginTop:'10px',padding:'10px',background:'#fff',borderRadius:'8px',border:'1px solid #cde8ce',wordBreak:'break-all'}}><div style={{fontSize:'11px',color:'#6b8a72'}}>Dev reset link (visible because SMTP not yet configured):</div><a href={forgotLink} style={{color:'#1b6a2f',fontWeight:700,fontSize:'12px'}}>{forgotLink}</a></div>}
            <div style={{marginTop:'8px',fontSize:'11px'}}><a href="/dev/outbox" style={{color:'#1b6a2f',textDecoration:'underline'}}>View all sent emails in dev outbox</a> — or check your inbox when Gmail is configured.</div>
          </div>
